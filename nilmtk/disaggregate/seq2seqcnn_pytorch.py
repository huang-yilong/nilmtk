...
class seq2seqcnn_Pytorch(nn.Module):
    ...
    def forward(self, x):
        ...
        return x.view(-1, self.seq_length)
...
class Seq2SeqCNN(Disaggregator):
    ...
    def partial_fit(self, train_main, train_appliances, pretrain=False, do_preprocessing=True,
                    pretrain_path="./seq2seqcnn_pre_state_dict.pkl", **load_kwargs):
        ...
        for appliance_name, power in train_appliances:
            if appliance_name not in self.models:
                ...
                if pretrain is True:
                    self.models[appliance_name].load_state_dict(
                        torch.load("./" + appliance_name + "_seq2seqcnn_pre_state_dict.pt"))
            ...
            self.models[appliance_name].load_state_dict(
                torch.load("./" + appliance_name + "_seq2seqcnn_best_state_dict.pt"))
    ...
    def call_preprocessing(self, mains_lst, submeters_lst, method):
        ...
        if method == 'train':
            ...
            for mains in mains_lst:
                ...
                mains = self.normalize_data(mains.values, sequence_length, mains.values.mean(), mains.values.std(),
                                            True)
