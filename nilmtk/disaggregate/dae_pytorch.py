...
class DAE_Pytorch(nn.Module):
    def __init__(self, sequence_length):
        ...
        self.dense = nn.Sequential(nn.Linear(8 * (sequence_length - 3), 8 * (sequence_length - 3)), nn.ReLU(True),
                                   nn.Linear(8 * (sequence_length - 3), 128), nn.ReLU(True),
                                   nn.Linear(128, 8 * (sequence_length - 3)), nn.ReLU(True))
...
class DAE(Disaggregator):
    ...
    def partial_fit(self, train_main, train_appliances, pretrain=False, do_preprocessing=True,
                    pretrain_path="./dae_pre_state_dict.pkl", **load_kwargs):
            ...
            if appliance_name not in self.models:
                ...
                if pretrain is True:
                    self.models[appliance_name].load_state_dict(
                        torch.load("./" + appliance_name + "_dae_pre_state_dict.pt"))
